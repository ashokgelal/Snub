...
class VisualStudioProjectDetector: ProjectTypeDetector {
    func detect(fileExtensions: [String]) -> ProjectDetectionResult? {
        ...
        if detectedTypes.count > 0 {
            DDLogVerbose("Detected \(detectedTypes.count) Visual Studio project type")
            return ProjectDetectionResult(id: "VisualStudio", projectType: "Visual Studio", confidencePercent: Double(detectedTypes.count) * 100.0/Double(knownExtensions.count))
        }
        return nil
    }
}
